...
import { ScanOutlined, MessageOutlined } from "@ant-design/icons";
    ...
    return (
        <Layout style={{ height: '100vh' }}>
            <div className="Header"  >
                <ScanOutlined />
                <div className="SearchBox" style={{ flex: 1, marginLeft: '20px', marginRight: '20px' }}>
                    <input type="text" placeholder="请输入商品 如：手机" style={{ width: '100%', padding: '5px' }} />
                </div>
                <MessageOutlined />
            </div>
            <Layout style={{ flex: 1 }}>
                ...
                <Layout style={{ height:'100%',marginLeft: 200, marginTop: 50, padding: '24px', background: 'rgba(255,255,255,0.26)' }}> {/* Adjusted marginTop to accommodate header */}
                    <Content style={{ height: '100%', overflow: 'auto' }}>
                        <div className='allGoods'>
                            <Button type="Link"  onClick={handleAllProductsClick}>
                                所有商品 <span style={{ marginLeft: '5px' }}>></span><Icon type="right"  style={{ color: 'black' }} />
                            </Button>
                        </div>
                        <Row gutter={[16, 16]}>
                            {(subCategories[selectedCategory] || []).map(subCategory => (
                                <Col xs={8} sm={8} md={6} lg={6} key={subCategory.key}>
                                    <div style={{ textAlign: 'center' }}>
                                        <img src="https://picsum.photos/400/200?random=1" alt={subCategory.title} style={{ width: '90px', height: '90px' }} />
